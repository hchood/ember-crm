App.LeadsController = Ember.ArrayController.extend({
  ...
  //  leads property looks for search string.  if it exists, returns
  //  searchedLeads, otherwise returns array of models it's wrapping.
  leads: function() {
    return this.get('search') ? this.get('searchedLeads') : this;
  }.property('search', 'searchedLeads'),

  // runs filter on the array of leads & returns leads where the
  // full name includes the search string.
